...
% angle delta top
\draw[dashed] (0, \dist) -- (0,\dist+2.7);
\draw[dashed] (0, \dist) -- ({-sin(\deltavar)*2.7},{\dist+2.7)});
\draw[color=black] (0,\dist+2.3) arc (90:90+0.9*\deltavar:2.3);
\draw (-0.27, \dist+1.7) node[black] {\Huge $\delta$};

 %angle delta left
\draw[color=black] ({\ax+2.7},0) arc (0:1.3*\deltavar:2);
\draw (\ax+2.2, 0.4) node[black] {\Huge $\delta$};
 
% circle 
 \draw[ ->, orange, line width=3] (0,0) arc (0:90:-\ax); 
