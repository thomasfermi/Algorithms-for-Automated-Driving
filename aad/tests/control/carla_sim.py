...
def main(use_lane_detector=False, ex=False, save_video=False, half_image=False):
    ...
    if use_lane_detector and not ex:
        ...
        from aad.exercises.lane_detection.lane_detector import LaneDetector
    if ex:
    ...
    try:
        ...
        if use_lane_detector:
            ...
            if not ex:
                ld = LaneDetector(
                    model_path=Path("aad/solutions/lane_detection/fastai_model.pth").absolute(),
                    cam_geom=cg,
                )
            else:
            ...
            cam_windshield_transform = carla.Transform(
                carla.Location(x=0.5, z=cg.height), carla.Rotation(pitch=cg.pitch_deg)
            )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--ld",
        action="store_true",
        help="Use reference trajectory from your LaneDetector class",
    )
