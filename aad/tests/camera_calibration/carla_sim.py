...
from aad.util.carla_util import (
    ...
    carla_img_to_array,
    CarlaSyncMode,
    get_weather_clear_noon,
    draw_image_np,
    should_quit,
)
from aad.util.geometry_util import dist_point_linestring
import argparse
import cv2
import copy
...
def get_trajectory_from_map(m, vehicle):
    ...
    traj = np.array([np.array([*carla_vec_to_np_array(x.transform.location), 1.0]) for x in wps]).T
...
def main(yaw_deg=0, pitch_deg=0, ex=False, save_video=False, half_image=False):
    # Imports
    if ex:
        # from aad.exercises.camera_calibration.calibrated_lane_detector import CalibratedLaneDetector
        from aad.exercises.lane_detection.camera_geometry import CameraGeometry
        from aad.exercises.control.pure_pursuit import PurePursuitPlusPID
    else:
        from aad.solutions.camera_calibration.calibrated_lane_detector import CalibratedLaneDetector
        from aad.solutions.lane_detection.camera_geometry_numba import CameraGeometry
        from aad.solutions.control.pure_pursuit import PurePursuitPlusPID
    ...
    display = pygame.display.set_mode(main_image_shape, pygame.HWSURFACE | pygame.DOUBLEBUF)
    ...
    weather_preset = get_weather_clear_noon()
    ...
    try:
        ...
        if not ex:
            model_path = Path(__file__).parent.parent / "solutions" / "lane_detection" / model_filename
            ld = CalibratedLaneDetector(model_path=str(model_path.absolute()), cam_geom=cam_geom, calib_cut_v=200)
        else:
        ...
        cam_windshield_transform = carla.Transform(
            carla.Location(x=0.5, z=cg.height), carla.Rotation(pitch=pitch_deg, yaw=yaw_deg)
        )
        ...
        with CarlaSyncMode(world, *sensors, fps=FPS) as sync_mode:
            while True:
                ...
                throttle, steer = controller.get_control(traj, speed, desired_speed=25, dt=1.0 / FPS)
                ...
                dy = 20
                texts = [
                    "FPS (real):             {}".format(int(clock.get_fps())),
                ...
                for it, t in enumerate(texts):
                    display.blit(font.render(t, True, (0, 0, 0)), (image_rgb.shape[1] - 270, 20 + dy * it))
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Runs Carla simulation with your control algorithm and the calibrated lane detector.",
    ...
    parser.add_argument(
        "--half_image",
        action="store_true",
        help="Pass images with (width, height) = (512,256) to lane detector instead of the default (1024,512). This will speed up the simulation, but might hurt accuracy.",
    )
