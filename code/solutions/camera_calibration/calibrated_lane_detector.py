...
class CalibratedLaneDetector(LaneDetector):
    def __init__(self, calib_cut_v = 300, cam_geom=CameraGeometry(), model_path='./lane_segmentation_model.pth', 
    ...
    def _fit_line_v_of_u(self, probs):
        # TODO: maybe we should make sure that degree 1 polynomial is a good fit here? 
        # Otherwise the image shall not be used for calibration
        probs_flat = np.ravel(probs[self.calib_cut_v:, :])
        mask = probs_flat > 0.3
        coeffs = np.polyfit(self.uv_grid[:,0][mask], self.uv_grid[:,1][mask], deg=1, w=probs_flat[mask])
